...
<template>
        ...
        <div class="w-full md:w-1/2">
          ...
          <div class="mt-4">
            ...
            <!-- ui只顯示唯一的尺寸選項，ui調整格式，ui調整排序-->
             <!-- cursor-not-allowed -->
              <button 
                v-for="(variant, index) in availableSizes" 
                :key="index" 
                :class="[
                  'px-4 py-2 font-medium rounded-lg shadow-md transition-all duration-300',
                  selectedSize === variant.size ? 'ring-2 ring-black bg-orange-500 text-white' : 'bg-gray-200 text-gray-700',
                  variant.count === 0 ? 'bg-gray-400 ' : 'hover:bg-orange-600'
                ]"
  ...
  <div class="flex items-center">
    <button 
      ...
      :disabled="isSoldOut"
      class="px-3 py-1 border border-gray-400 rounded-l-lg"
      :class="{'bg-gray-300 cursor-not-allowed': isSoldOut, 'bg-white hover:bg-gray-200': !isSoldOut}"
    >-</button>
    
    <input 
      type="text" 
      v-model="quantity" 
      class="w-12 text-center border-t border-b border-gray-400"
      :disabled="isSoldOut"
      :class="{'bg-gray-300': isSoldOut, 'bg-white': !isSoldOut}"
    />
    
    <button 
      @click="incrementQuantity"
      :disabled="isSoldOut"
      class="px-3 py-1 border border-gray-400 rounded-r-lg"
      :class="{'bg-gray-300 cursor-not-allowed': isSoldOut, 'bg-white hover:bg-gray-200': !isSoldOut}"
    >+</button>
  </div>
            <button 
  ...
  export default {
    ...
    //3/27
    isSoldOut() {
      return this.selectedVariant ? this.selectedVariant.count === 0 : false;
    }
    ...
    methods: {
      ...
      //327
      incrementQuantity() {
      if (this.selectedVariant && this.selectedVariant.count > 0) {
        this.quantity++;
      }
    },
    decrementQuantity() {
      if (this.selectedVariant && this.selectedVariant.count > 1) {
        this.quantity--;
      }
    },
      ...
      //3/27
  //     updateVariants() {
  //   const order = ['0 - 3 Months', '3 - 6 Months', '6 - 12 Months', 'F']; // 自訂排序
  //   this.availableSizes = this.product.variants
  //     .filter((variant) => variant.color === this.selectedColor)
  //     .map((variant) => ({
  //       size: variant.size.trim().replace(/\s*-\s*/g, ' - '), // 格式化尺寸
  //       count: variant.count
  //     }))
  //     .sort((a, b) => order.indexOf(a.size) - order.indexOf(b.size)); // 排序

  //   const firstAvailable = this.availableSizes.find(variant => variant.count > 0);
  //   this.selectedSize = firstAvailable ? firstAvailable.size : null;
  //   this.selectSize(this.selectedSize);
  // },
  
  updateVariants() {
  const allSizes = [...new Set(this.product.variants.map(v => v.size.trim().replace(/\s*-\s*/g, ' - ')))];

  const colorVariants = this.product.variants.filter(variant => variant.color === this.selectedColor);

  this.availableSizes = allSizes.map(size => {
    const matchedVariant = colorVariants.find(variant => variant.size === size);
    return {
      size,
      count: matchedVariant ? matchedVariant.count : 0
    };
  });
   // 不預設選擇尺寸，讓使用者手動選擇
  ...
  // const firstAvailable = this.availableSizes.find(variant => variant.count > 0);
  // this.selectedSize = firstAvailable ? firstAvailable.size : this.availableSizes[0]?.size;
  // this.selectSize(this.selectedSize);
},
