...
export const useCartStore = defineStore("cart",{
    state:() =>({
        cartData:JSON.parse(localStorage.getItem("cartData")) || [],//當前用戶的購物車內容
...
this.cartItems = this.processCartItems(
    this.backendCart.items.map(item => {
        ...
        //前端單一商品的格式資料(前端UI所需的 cartItems 格式，為購物清單專用) 3/2
...
//         },
        ...
        increaseQuantity(index) {
            ...
            this.cartItems[index].subTotal = this.cartItems[index].price * this.cartItems[index].quantity;//更新小計
            this.updateSelectedTotal(); // 更新被選商品的總金額
            this.syncCartToLocalStorage(); // 同步到 localStorage
        ...
        //商品詳細頁面的加入購物車功能  原本
        // async addItemToCart(product, selectedSize, selectedColor, quantity = 1) {
        //     const authStore = useAuthStore();
        //     const token = authStore.token;
        //     const userId = authStore.id;

        //     // ✅ 判斷該商品是否需要 size 和 color
        //     const requiresSize = product.sizeOptions && product.sizeOptions.length > 0;
        //     const requiresColor = product.colorOptions && product.colorOptions.length > 0;

        //      // ✅ 只有當商品需要 size 或 color 時，才進行檢查
        //     if ((requiresSize && !selectedSize) || (requiresColor && !selectedColor)) {
        //         alert('請選擇尺寸和顏色');
        //         return;
        //     }

        //     // 取得 localStorage 最新購物車數據
        //     this.cartItems = JSON.parse(localStorage.getItem('cartItems')) || [];
        
        //     // 🔹 檢查 localStorage 是否已有相同商品（相同 ID、尺寸、顏色）
        ...
        //     if (existingItem) {
        //         existingItem.quantity = quantity; // 重新設定數量
        //     } else {
        //         // 前端localStorage顯示的格式
        //         existingItem = {
        //             userId,// 記錄是哪個使用者的購物車
        //             product_Id: product.id,
        //             Name: product.name,
        //             [product.price > 0 ? 'Price' : 'OriginalPrice']: product.price || product.OriginalPrice, // 動態設置鍵名
        //             size: selectedSize,
        //             color: selectedColor,
        //             quantity,// 這裡的 quantity 來自參數
        //         };
        ...
        //         // 2️⃣ 檢查 API 是否已有相同商品
        //         const apiExistingItem = cartData.find(
        //             (cartItem) =>
        //                 //cartItem.user_id === userI
        ...
        //         if (apiExistingItem) {
        //             // ✅ 如果商品已存在，則 `PATCH` 更新數量
        //             await axios.patch(
        //                 `https://204ed3432b06d7af.mokky.dev/carts/${apiExistingItem.id}`,
        //                 {quantity:existingItem.quantity},
        //                 { headers: { Authorization: `Bearer ${token}` } }
        //             );
        //         } else {
        //             // ✅ 如果商品不存在，則 `POST` 新增該商品
        //             const priceKey = existingItem.Price !== undefined ? 'Price' : 'OriginalPrice';
                
        //             await axios.post(
        //                 'https://204ed3432b06d7af.mokky.dev/carts',
        //                 {
        //                     // user_id:userId
        //                     userId:userId, // API 需要的 `user_id`
        //                     product_Id: existingItem.product_Id,
        //                     Name: existingItem.Name,
        //                     [priceKey]: existingItem[priceKey], //動態設置鍵名 直接取 existingItem 裡的對應價格
        ...
        //     if (existingItem) {
        //         existingItem.quantity = quantity; // 重新設定數量
        //         existingItem.subTotal = (existingItem.price || existingItem.OriginalPrice) * quantity;
        //     } else {
        //         existingItem = {
        //             product_Id: product.id,
        //             quantity,
        ...
        //     // 更新 total 與 final_total
        //     existingCart.total = existingCart.items.reduce((sum, item) => sum + item.subTotal, 0);
        //     existingCart.final_total = existingCart.total;
        ...
        //         // 🔹 檢查該商品是否已存在購物車
        //         let existingItem = existingCart.items.find(
        //             item => item.product_Id === product.id && item.size === selectedSize && item.color === selectedColor
        //         );
        
        //         if (existingItem) {
        //             existingItem.quantity = quantity; // 直接增加數量
        //             existingItem.subTotal = (existingItem.price || existingItem.OriginalPrice) * existingItem.quantity;
        //         } else {
        //             existingCart.items.push(newItem);
        //         }
        
        //         // 🔹 計算新 total / final_total
        //         existingCart.total = existingCart.items.reduce((sum, item) => sum + item.subTotal, 0);
        //         existingCart.final_total = existingCart.total;
        
        //         // 🔄 更新購物車
        //         try {
        //             await axios.patch(`https://204ed3432b06d7af.mokky.dev/cartsData/${existingCart.id}`, existingCart, {
        //                 headers: { Authorization: `Bearer ${token}` },
        //             });
        //             console.log(`購物車 (ID: ${existingCart.id}) 更新成功！`);
        //         } catch (error) {
        ...
        //             final_total: newItem.subTotal,
        //             status: false,
        //             orderStatus: false,
        //         };
        
        //         try {
        //             await axios.post("https://204ed3432b06d7af.mokky.dev/cartsData", newCart, {
        //                 headers: { Authorization: `Bearer ${token}` },
        //             });
        //             console.log("購物車新增成功！");
        //         } catch (error) {
        //             console.error("購物車新增失敗", error);
        //         }
        //     }
        ...
        async addItemToCart(product, selectedSize, selectedColor, quantity = 1) { 
            ...
            let existingItem = cartData.items.find(
                item => item.product_Id === product.id && item.product.color === selectedColor && item.product.size === selectedSize
            );
        
            if (existingItem) {
                existingItem.quantity = quantity; // 直接增加數量
                existingItem.subTotal = (existingItem.price || existingItem.product.OriginalPrice) * existingItem.quantity;
            } else {
            ...
            // 🔄 更新購物車 API 與 LocalStorage
            try {
                const response = await axios.patch(`https://204ed3432b06d7af.mokky.dev/cartsData/${userId}`, cartData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
        ...
        //             quantity: item.quantity,
        //             price: item.price || item.OriginalPrice, // 商品價格
        //             subTotal: (item.price || item.OriginalPrice) * item.quantity,//單一商品小計
        //             final_total: (item.price || item.OriginalPrice) * item.quantity, // 最後價格如果有折扣可調整
        //         };
        ...
        //     } catch (error) {
        //         console.error('無法獲取購物車資料', error);
        //         return; // 如果無法取得購物車數據，停止執行
        //     }
        //     // 歷 cartItems，轉換為 /carts 格式
        //     for (const item of this.cartItems) {

        //         // 檢查 API 中是否已經有相同的商品（根據 userId 和 product_Id）
        //         const existingItem = existingCart.find(
        //             //cartItem.userId === item.userId
        //             cartItem => cartItem.userId === userId && cartItem.product_Id === item.product_Id
        //         );
        ...
        //                     headers: { Authorization: `Bearer ${token}` }
        //                 });
        //                 console.log(`更新商品 ${item.product_Id} 成功`);
        //             } catch (error) {
        //                 console.error(`更新商品 ${item.product_Id} 失敗`, error);
        //             }
        //         } else {
        //             // 🆕 商品不存在，使用 POST 新增
        //             try {
        ...
        //                     headers: { Authorization: `Bearer ${token}` }
        //                 });
        //                 console.log(`新增商品 ${item.product_Id} 成功`);
        //             } catch (error) {
        //                 console.error(`新增商品 ${item.product_Id} 失敗`, error);
        //             }
        //         }
               
        //     }
        
        //     // API 同步後，清除 localStorage 的暫存(避免長期存儲造成不同步問題)
        //     localStorage.removeItem('cartItems');
        // },
        // async syncCartToAPI() {
        //     const authStore = useAuthStore();
        ...
        //         if (existingItem) {
        //             // 如果商品已存在，則更新數據(id是自動生成，要用userId和商品id找出生成id)
        //             try {
        //                 await axios.patch(`https://204ed3432b06d7af.mokky.dev/carts/${existingItem.id}`, payload, {
        //                     headers: { Authorization: `Bearer ${token}` }
        //                 });
        ...
        async syncCartToAPI() {
            ...
            for (const item of this.cartData) {
                const existingItem = existingCart.find(
                    existingCartItem => existingCartItem.userId === userId && existingCartItem.product_Id === item.product_Id
                );
        ...
        //商品詳細頁面的加入購物車功能 同步到 /cartsData
        // async  syncCartDataToAPI() {
        //     const cartItems = JSON.parse(localStorage.getItem('cartItems')) || [];
        //     const authStore = useAuthStore();
        //     const token = authStore.token;
        
        //     let formattedCart = {
        //         userId: authStore.id,//user_id
        //         items: cartItems.map(item => ({
        //             product_Id: item.product_Id,
        //             quantity: item.quantity,
        //             price: item.price || item.OriginalPrice,//商品金額
        //             subTotal: (item.price || item.OriginalPrice) * item.quantity,//小計
        //             product: {
        //                 id: item.product_Id,
        //                 name: item.Name,
        //                 imgurl: item.imgurl,
        ...
        //             selected: false,
        //         })),
        //         total: cartItems.reduce((sum, item) => sum + (item.price || item.OriginalPrice) * item.quantity, 0),//所有商品金額
        //         final_total: cartItems.reduce((sum, item) => sum + (item.price || item.OriginalPrice) * item.quantity, 0), // 這裡可以改成計算折扣後的價格
        //         status: false,
        //         orderStatus: false,
        //     };
        
        //     await axios.post('https://204ed3432b06d7af.mokky.dev/cartsData', formattedCart, {
        //         headers: { Authorization: `Bearer ${token}` }
        //     });
        ...
        //     // 從 localStorage 取得 cartItems
        //     const cartItems = this.cartItems;
        //     if (cartItems.length === 0) return; // 沒有商品就不執行
        
        //     // 取得 API 中的購物車內容(設 null 比 設 [] 更適合 用來表示「購物車是否存在」因為這裡的 existingCart 不是陣列，而是一個物件（購物車資料）。)
        //     let existingCart = null;
        //     try {
        //         const response = await axios.get("https://204ed3432b06d7af.mokky.dev/cartsData", {
        //             headers: { Authorization: `Bearer ${token}` },
        ...
        //     // 整理新的購物車資料
        //     let formattedCart = {
        //         userId: userId, // user_id authStore.id
        //         items: cartItems.map(item => ({
        //             product_Id: item.product_Id,
        //             quantity: item.quantity,
        //             price: item.price || item.OriginalPrice, // 商品金額
        ...
        //             selected: false,
        //         })),
        //         total: cartItems.reduce((sum, item) => sum + (item.price || item.OriginalPrice) * item.quantity, 0), // 總金額
        //         final_total: cartItems.reduce((sum, item) => sum + (item.price || item.OriginalPrice) * item.quantity, 0), // 折扣後金額
        //         status: false,
        //         orderStatus: false,
        //     };
        ...
        //     // 🔄 購物車已存在 -> 更新 (PATCH)
        //     if (existingCart) {
        //         for (let newItem of formattedCart.items) {
        //             let existingItem = existingCart.items.find(item => item.product_Id === newItem.product_Id);
        //             if (existingItem) {
        //                 // 更新數量與小計
        ...
        //     if (this.cartItems.length === 0) return; // 沒有商品就不執行
        ...
        //             selected: false,
        //         })),
        //         total: this.cartItems.reduce((sum, item) => sum + (item.price || item.OriginalPrice) * item.quantity, 0),
        //         final_total: this.cartItems.reduce((sum, item) => sum + (item.price || item.OriginalPrice) * item.quantity, 0),
        //         status: false,
        //         orderStatus: false,
        //     };
        ...
        //         // 更新購物車
        //         try {
        //             await axios.patch(`https://204ed3432b06d7af.mokky.dev/cartsData/${existingCart.id}`, formattedCart, {
        //                 headers: { Authorization: `Bearer ${token}` },
        //             });
        //             console.log(`購物車 (ID: ${existingCart.id}) 更新成功！`);
        //         } catch (error) {
