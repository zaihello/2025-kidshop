    ...
    async getProducts() {
      try {
          ...
          this.products = response.data
            // Boolean(product.is_enabled)避免 localStorage 內的舊資料還是 1/0，載入時確保轉換，讓 0 變 false，1 變 true
              .filter(product => Boolean(product.is_enabled)) // 只保留 is_enabled === true 的商品
              .map(product => ({
                  id: product.id || null,
                  name: product.name || "",
                  ...
                  // is_enabled: product.is_enabled, // 保持 is_enabled 為數字 (1 或 0)
  ...
  getters: {
    ...
    filteredProducts(state) {
        let filtered = state.products.filter((item) => {
            ...
            // 尺寸篩選
            const matchesSize = !size || item.size.includes(size);
    ...
    // colorClass 是一個函數，可以根據顏色名稱返回 Tailwind 類名
    colorClass: (state) => (color) => state.colors[color] || 'bg-gray-200',
