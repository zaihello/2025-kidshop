...
export default{
    ...
    methods:{
      ...
      // },
      handleLogout() {
        this.adminAuthStore.clearAuth();
        this.$router.push("/admin/login"); // 登出後導向後台登入頁面
      },
        //漢堡選單開關
      toggleSidebar() {
    ...
    <div class="hidden lg:block lg:fixed lg:top-0 lg:left-0 lg:right-0 z-30 bg-orange-100 px-4 py-5">
      ...
      <div class="lg:flex lg:items-center lg:justify-between px-4 py-5 w-full 2xl:w-3/4 2xl:m-auto">
        ...
        <div class="space-x-4 text-lg text-gray-800">
          <router-link to="/shop" class="hover:text-blue-600">商店瀏覽</router-link>
        </div>

        <!-- 右側區塊：登入和購物車按鈕 -->
        <div class="space-x-4">
          <!-- 登入時顯示 -->
          <ul v-if="adminAuthStore.isLoggedIn" class="flex gap-4">
            <li>
