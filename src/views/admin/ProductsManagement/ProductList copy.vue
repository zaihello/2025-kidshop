...
<template>
    ...
    <td class="px-4 py-2">
        <img :src="product.variants[0]?.imgurl" alt="商品圖片" class="w-16 h-16 object-cover rounded">
    </td>
...
<!-- 變體 (variants) 折疊區域 -->
<tr v-if="expandedProduct === product.id" v-for="variant in product.variants" :key="variant.id" class="bg-gray-100">
    <td></td> <!-- 空一格對齊 -->
    <td><img :src="variant.imgurl" class="w-12 h-12 object-cover rounded"></td>
    <td colspan="1"></td> <!-- 空一格對齊 -->
    <td class="px-4 py-2">{{ variant.size.join(", ") }}</td>
    <td class="px-4 py-2">{{ variant.color.join(", ") }}</td>
    <td class="px-4 py-2">{{ variant.mark.join(", ") || '無' }}</td>
